...
const MultiStepQuiz = ({
  ...
  const handleNext = () => {
    ...
    if (typeof window !== 'undefined' && (window as any).ym) {
      if (currentStep === 1) {
        (window as any).ym(106851071, 'reachGoal', 'date_selected');
      } else if (currentStep === 2) {
        (window as any).ym(106851071, 'reachGoal', 'city_entered');
      } else if (currentStep === 3) {
        (window as any).ym(106851071, 'reachGoal', 'time_selected');
      }
    }
        ...
        <div className="flex-1 flex flex-col px-4 pb-8 overflow-hidden">
          <div className="w-full max-w-md mx-auto space-y-6 animate-fade-in flex flex-col h-full">
            ...
            <div className="flex-shrink-0">
              <Button onClick={() => window.open('https://t.me/FreeMoonGorosskop_Bot', '_blank')} className="w-full h-14 text-base font-semibold bg-gradient-gold hover:scale-105 transition-all duration-300 shadow-[0_0_30px_rgba(255,216,138,0.6)] hover:shadow-[0_0_50px_rgba(255,216,138,0.9)] border-0">
                Получить анализ ✨
              </Button>
            </div>
          </div>
